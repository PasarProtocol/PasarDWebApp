...
export default function HomeAssetCard() {
  ...
  React.useEffect(() => {
    fetchFrom('api/v2/sticker/getLatestPurchasedToken')
      .then((response) => {
        response
          .json()
          .then((jsonAsset) => {
            if (jsonAsset.data) setAsset(jsonAsset.data);
          ...
          <>
            ...
            <Stack direction="row" sx={{ py: 2 }}>
              <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                <Link
                  to={`/profile/others/${asset.royaltyOwner}`}
                  component={RouterLink}
                  sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'inherit' }}
                >
