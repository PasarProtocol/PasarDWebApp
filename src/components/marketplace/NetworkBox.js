...
const NetworkBox = (props)=>{
    ...
    React.useEffect(()=>{
        if(current==='ESC' || current==='ETH')
            setChainIndex(current==='ESC'?0:1)
    }, [current])
    ...
    return(
        ...
        >
            <Stack direction="row" alignItems='center'>
                ...
                <Typography variant="subtitle1" color={current?'text.primary':'text.secondary'}>{chainTypes[chainIndex].name}</Typography>
            </Stack>
        </Paper>
    )
}
